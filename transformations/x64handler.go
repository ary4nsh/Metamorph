...
func generateRandomKeyX64(valueSize int, originalValue int64) int64 {
	...
	case 32:
		if originalValue >= 0 {
			...
			// Generate a random int32 in the full range by using Int31 and subtracting half the range
			key = int64(rand.Int31()) - (1 << 30)
		}
	case 64:
		if originalValue >= 0 {
			key = rand.Int63()
		} else {
			// Generate a random int64 in the full range by using Int63 and subtracting half the range
			key = rand.Int63() - (1 << 62)
		}
	}
